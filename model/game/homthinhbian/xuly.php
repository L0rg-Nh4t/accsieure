...
if($row['giatien'] > $user['money']) {
    die("<script>swal('Lỗi', 'Bạn Không Đủ Tiền để mua Hòm Tiếp Tế!', 'error');</script>");
}



if(!$row['id']) {
    die("<script>swal('Lỗi', 'Hòm Tiếp Tế Không Còn Tồn Tại!', 'error');</script>");
}
    ...
    mysqli_query($kun->connect_db(), "INSERT INTO `user_history_system` (`username`, `action`, `action_name`, `sotien`, `mota`, `time`) VALUES ('".$user['username']."', 'Hòm Tiếp Tế', 'Hòm Tiếp Tế', '-".number_format($row['giatien'])."đ', 'Nhận được ".$row['kimcuong']." Kim Cương', '".time()."')");
    	// Update Tiền User
    ...
    echo "<script>swal('Thông Báo!', 'Bạn Nhận Được ".$row['kimcuong']." Kim Cương Từ Hòm Tiếp Tế Này!', 'success');</script>";
?>
